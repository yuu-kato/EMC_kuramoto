...
//
// for apptainer in rx:
// compile: apptainer exec 241112.sif g++ -fopenmp -O3 filename.cpp
// execute: apptainer exec --env OMP_NUM_THREADS=20 filename.sif > log.txt ./a.out
//
// for local
// compile: g++ -fopenmp -O3 filename.cpp
// execute: OMP_NUM_THREADS=8 > log.txt ./a.out
//
// revision history:
// ver. 24-11-10 (fix seeds for both data generation and estimation (before parallelization))
// ver. 24-11-13 (delete unnecessary lines and output files)
// ver. 24-11-14 (Gaussian proposal distribution for Metropolis sampling)
// ver. 24-11-15 (Change step size during the burn-in period)
// ver. 24-12-05 (Revise data_generate(), calculate free energy, refine the code)
// ver. 24-12-25 (revise prior (from < to <=))
...
double R_t(double t, double K, double gamma){
    double R;
    double R_0 = 1.0;
    R = exp(-gamma * t + 0.5 * K * t) / sqrt(1.0/R_0/R_0 + (exp(-2.0 * gamma * t + K * t) - 1.0) * K / (-2*gamma + K)); //typo corrected (20240605 by yKato)
    return R;
}
...
void EMC(int L, const vector<double> &b, int N, const vector<double> &t_data, const vector<double> &R_data, vector<vector<double>> s, int simulation_size, int burn_in, double target_accept, double range_accept, int seed, int exchange_freq, double alpha_gamma, double beta_gamma, double alpha_delta_gamma, double beta_delta_gamma){
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ofstream fout_1("./output_files_0001/parameter.csv");
    fout_1 << "mc_step" << ",";
    for (int l = 0; l < L; l++){
    ...
    ofstream fout_2("./output_files_0001/error.csv");
    fout_2 << "mc_step" << ",";
    for (int l = 0; l < L; l++){
    ...
    ofstream fout_3("./output_files_0001/exchange_rate.csv");
    ofstream fout_4("./output_files_0001/acceptance_rate.csv");
    ofstream fout_5("./output_files_0001/step_size.csv");
    fout_3 << "mc_step" << ",";
    fout_4 << "mc_step" << ",";
    fout_5 << "mc_step" << ",";
    for (int l = 0; l < L; l++) {
    ...
    ofstream fout_6("./output_files_0001/MAP.csv");
    fout_6 << "l" << ",";
    for (int i = 0; i < L; i++) {
        fout_6 << i << ",";
    }
    ...
    ofstream fout_7("./output_files_0001/free_energy.txt");
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int main() {
    ...
    // create output files
    mkdir("./output_files_0001", 0755); // 0755 means 'allowing the owner to write, read, execute, while allowing others only to read and execute'
    ofstream fout_1("./output_files_0001/data.txt");
    ofstream fout_2("./output_files_0001/true_parameters.csv");
