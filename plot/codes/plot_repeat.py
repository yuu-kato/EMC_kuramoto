...
ver. 24-12-09 original
ver. 24-12-19 summarize in one figure
'''
...
labelsize = 25
labelsize2 = 20 # ylabel for variance
legendsize = 25
panellabelsize = 30
config = {
    'font.size': 14,
...
def plot_rep(param, subj_list, K_MAP_ave, gamma_MAP_ave, K_MAP_std, gamma_MAP_std, K_MAP_dif_ave, gamma_MAP_dif_ave, K_MAP_dif_std, gamma_MAP_dif_std, K_var_ave, gamma_var_ave, K_var_std, gamma_var_std, XLIM, xlabel, savefilename):
    ...
    ax1.errorbar(subj_list, K_MAP_ave, yerr=K_MAP_std, color = 'blue', marker=".", markersize=20, label = r"$\hat{K}$", capthick=1, capsize=5, lw=3)
    ...
    ax2.errorbar(subj_list, gamma_MAP_ave, yerr=gamma_MAP_std, color = 'lime', marker=".", markersize=20, label = r"$\hat{\gamma}$", capthick=1, capsize=5, lw=3)
    ...
    ax3.errorbar(subj_list, K_MAP_dif_ave, yerr=K_MAP_dif_std, color = 'blue', marker=".", markersize=20, capthick=1, capsize=5, lw=3, label = r"$|K_{\rm true}-\hat{K}|$")
    ...
    ax4.errorbar(subj_list, gamma_MAP_dif_ave, yerr=gamma_MAP_dif_std, color = 'lime', marker=".", markersize=20, capthick=1, capsize=5, lw=3, label = r"$|\gamma_{\rm true} - \hat{\gamma}|$")
    ...
    ax5.errorbar(subj_list, K_var_ave, yerr=K_var_std, color = 'blue', marker=".", markersize=20, label = r'${\rm Var}\: p(K|D,b_{\hat{l}})$', capthick=1, capsize=5, lw=3)
    ax5.set_xscale('log')
    ax5.set_xlim(XLIM)
    ...
    ax5.set_ylabel('Variance of \n posterior distribution', fontsize = labelsize, x = -0.05)

    ax6 = fig.add_subplot(326)
    ax6.errorbar(subj_list, gamma_var_ave, yerr=gamma_var_std, color = 'lime', marker=".", markersize=20, label = r'${\rm Var}\: p(\gamma|D,b_{\hat{l}})$', capthick=1, capsize=5, lw=3)
    ax6.set_xscale('log')
    ax6.set_xlim(XLIM)
